...
import { defaultCharacters, getCharacters, saveCharacters, type StoryCharacter } from '../../store/useCharacterCatalog'

export default function CharacterManager({ onClose }: { onClose: () => void }) {
  const [characters, setCharacters] = useState<StoryCharacter[]>(getCharacters())
  const [query, setQuery] = useState('')
  const [selected, setSelected] = useState<StoryCharacter | null>(characters[0] || null)
  ...
  const persist = (next: StoryCharacter[]) => { setCharacters(next); saveCharacters(next) }
  const addCharacter = () => {
    if (!name.trim()) return
    const character: StoryCharacter = { id: `${Date.now()}`, name: name.trim(), role: 'New Character', color: '#a855f7', status: 'active', bio: 'Add character background and story details.', tags: [] }
    const next = [...characters, character]; persist(next); setSelected(character); setName(''); setShowForm(false)
  }
  const removeCharacter = () => {
    if (!selected || !confirm(`Remove ${selected.name} from the cast?`)) return
    const next = characters.filter(c => c.id !== selected.id); persist(next); setSelected(next[0] || null)
  }
    ...
    <div className="absolute inset-0 z-50 flex bg-black/70 backdrop-blur-sm">
      <section className="ml-auto flex h-full w-full max-w-5xl border-l border-white/10 bg-[#09090b] text-white shadow-2xl">
        <div className="flex w-80 flex-col border-r border-white/10 bg-[#0d0d0f]">
          ...
          <div className="flex-1 space-y-1 overflow-auto px-3 pb-3">{filtered.map(c => <button key={c.id} onClick={()=>setSelected(c)} className={`flex w-full items-center gap-3 rounded-xl p-3 text-left transition ${selected?.id===c.id?'bg-white/10':'hover:bg-white/5'}`}><span className="flex h-10 w-10 items-center justify-center rounded-full border" style={{color:c.color,borderColor:`${c.color}55`,background:`${c.color}15`}}><User size={18}/></span><span className="min-w-0 flex-1"><b className="block truncate text-xs">{c.name}</b><small className="text-[9px] uppercase tracking-wider text-white/35">{c.role}</small></span><span className={`h-2 w-2 rounded-full ${c.status==='active'?'bg-emerald-400':c.status==='dead'?'bg-red-500':'bg-amber-400'}`}/></button>)}</div>
        ...
        <div className="flex-1 overflow-auto p-8">{selected ? <><div className="flex items-start justify-between"><div className="flex items-center gap-4"><span className="flex h-16 w-16 items-center justify-center rounded-2xl border" style={{color:selected.color,borderColor:`${selected.color}55`,background:`${selected.color}15`}}><User size={30}/></span><div><h1 className="text-2xl font-black">{selected.name}</h1><p className="text-xs uppercase tracking-widest text-white/35">{selected.role} · {selected.status}</p></div></div><button onClick={removeCharacter} className="rounded-lg p-2 text-white/30 hover:bg-white/5 hover:text-red-400"><Trash2 size={16}/></button></div><p className="mt-8 max-w-2xl text-sm leading-6 text-white/55">{selected.bio}</p><div className="mt-8 grid gap-4 sm:grid-cols-3"><div className="rounded-2xl border border-white/10 bg-white/[0.03] p-5"><Heart size={18} className="text-pink-400"/><h3 className="mt-4 text-xs font-bold">Relationships</h3><p className="mt-1 text-[10px] text-white/35">Friends, enemies, family and rivals.</p></div><div className="rounded-2xl border border-white/10 bg-white/[0.03] p-5"><Brain size={18} className="text-violet-400"/><h3 className="mt-4 text-xs font-bold">Memories</h3><p className="mt-1 text-[10px] text-white/35">Important events this character remembers.</p></div><div className="rounded-2xl border border-white/10 bg-white/[0.03] p-5"><Shield size={18} className="text-emerald-400"/><h3 className="mt-4 text-xs font-bold">Continuity</h3><p className="mt-1 text-[10px] text-white/35">Track status and story appearances.</p></div></div><div className="mt-8"><h3 className="text-[10px] font-black uppercase tracking-widest text-white/40">Tags</h3><div className="mt-3 flex flex-wrap gap-2">{selected.tags.map(t=><span key={t} className="rounded-full border border-white/10 bg-white/5 px-3 py-1 text-[9px] uppercase text-white/50">{t}</span>)}</div></div></> : <div className="flex h-full items-center justify-center text-sm text-white/30">No characters yet</div>}</div>
      </section>
